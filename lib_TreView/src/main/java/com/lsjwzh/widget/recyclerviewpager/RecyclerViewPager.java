...
import com.mjiayou.treview.BuildConfig;
import com.mjiayou.treview.R;
...
public class RecyclerViewPager extends RecyclerView {
                    ...
                    if (mOnPageChangedListeners != null) {
                        for (OnPageChangedListener onPageChangedListener : mOnPageChangedListeners) {
                            if (onPageChangedListener != null) {
                                ...
                                int actualOldPosition = mPositionBeforeScroll % ((LoopRecyclerViewPagerAdapter) mViewPagerAdapter).getActualItemCount();
                                int actualNewPosition = getCurrentPosition() % ((LoopRecyclerViewPagerAdapter) mViewPagerAdapter).getActualItemCount();
                                onPageChangedListener.OnActualPageChanged(actualOldPosition, actualNewPosition);
                            }
                        }
                    }
                }
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        // treason@20151209
        getParent().requestDisallowInterceptTouchEvent(true); // 通知其父控件，现在进行的是本控件的操作，不允许拦截
    ...
    public interface OnPageChangedListener {
        ...
        void OnActualPageChanged(int oldPosition, int newPosition);
    }

    // treason@20151114
    private static final int DELAY_BANNER_SWITCH = 6000;
    ...
    private void startImageTimerTask() {
        isStop = false;
        // 图片滚动
        mHandler.postDelayed(mImageTimerTask, DELAY_BANNER_SWITCH);
    }
    ...
    private Runnable mImageTimerTask = new Runnable() {
        ...
        public void run() {
            if (mViewPagerAdapter != null) {
                ...
                if (!isStop) {  // if isStop=true //当你退出后 要把这个给停下来 不然 这个一直存 就一直在后台循环
                    mHandler.postDelayed(mImageTimerTask, DELAY_BANNER_SWITCH);
                }
            }
        }
